...
"$ROOT/scripts/render-kaia-service-rpc-conf.sh" "$ENV_FILE" "$OUT_DIR/conf/ksend.conf" >/dev/null
...
Next steps on the MBP:
  1. Place ksend.conf into the extracted Kaia package conf directory.
  ...
  4. Initialize the data directory with the service-chain genesis before starting the node.
  5. Start the SEN/endpoint process and verify rpc_modules, eth_chainId, and block progression.
EOF
